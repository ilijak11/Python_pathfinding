...
class HillClimber():
    ...
    def get_agent_path2(self, coin_distance):
        start_coin = {
            'ind' : 0,
            'cost' : 0
        }
    ...
    def get_agent_path(self, coin_distance):
        ...
        while len(pqueue) != 0:
                ...
                pqueue.insert(0, 
                    {
                        ...
                        'total_cost' : curr_coin['total_cost'] + coin_distance[curr_coin['ind']][0]
                    }
                )
                continue
            ...
            neighbours = [ {
                ...
                'total_cost' : curr_coin['total_cost'] + cost
            } for ind, cost in enumerate(coin_distance[curr_coin['ind']]) if ind not in curr_coin['path']]
            neighbours.sort(key = lambda x : (x['cost'], x['ind']))
            pqueue = neighbours + pqueue

        path = curr_coin['path']
        cost = curr_coin['total_cost']
...
class BFS():
    ...
    def get_agent_path(self, coin_distance):
        ...
        path = []
        next = [start_coin]
        curr_coin = start_coin
        cost = 0

        while len(path) != len(coin_distance):
            last_coin = curr_coin
            curr_coin = next.pop(0)
            cost += coin_distance[last_coin['ind']][curr_coin['ind']]
            path.append(curr_coin['ind'])
            next += [{
                'ind' : ind,
            } for ind in range(len(coin_distance)) if ind not in path]
    ...
    def get_agent_path(self, coin_distance):
        ...
        while len(path) != len(coin_distance):
            ...
            next = [{
                'ind' : ind,
                'cost' : cost
            } for ind, cost in enumerate(coin_distance[curr_coin['ind']]) if ind not in path] + next
...
if __name__ == '__main__':
    pos, matrix = load_map(sys.argv[1]) 
    for lst in matrix:
        print(lst)

    #agent - HillClimber, BestPermutation, BranchAndBound, AStar
    agent = AStar()
    cost_calc = BestPermutation()
    #cost_calc.calculate_path_cost([0,1,2,3,4,0], matrix)
    ...
    print("time: ", (e - s), "s")
    print("path: ", path)
    print("cost: ", cost_calc.calculate_path_cost(path, matrix))